...
def ensure_directory_exists(path):
    ...
    normalized_path = os.path.abspath(os.path.normpath(path))  # Normalize & get absolute path
    
    if not os.path.exists(normalized_path):
        # create directory if it doesn't exist
        os.makedirs(normalized_path)
        print(f"Directory created at {normalized_path}")
    else:
        print(f"Directory already exists at: {normalized_path}")
...
def save_results(df, messages_list, temperature, memory, feedback, instruction_type, expmnt_num, seed, \
    ...
    # Convert path_exp to an absolute, normalized path
    path_exp = os.path.abspath(os.path.normpath(path_exp))
    ...
    csv_filename = os.path.join(path_exp, f'expmnt_results_{feedback}_{instruction_type}_num_{num}_temp_{temp_str}_memory_{memory}_ntime_{n_steps}_nagents_{n_agents}_model_{gpt_str}.csv')
    ...
    pickle_filename = os.path.join(path_exp, f'expmnt_messages_{feedback}_{instruction_type}_num_{num}_temp_{temp_str}_memory_{memory}_ntime_{n_steps}_nagents_{n_agents}_model_{gpt_str}.pkl')
